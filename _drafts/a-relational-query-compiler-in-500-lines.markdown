...
There are a few outliers where Imp is up to 867x faster. The worst of them is [29a](https://gist.github.com/jamii/c36a0036503be18834a2127ba4e2e02c) which produces [this plan](https://explain.depesz.com/s/R628). It has underestimated the number of rows by 4890x, leading to a poor choice of query plan. But, despite their claims to innocence, it's pretty clear the author deliberately crafted this query to confuse Postgres with cross-constraint correlations. Many of the queries contain correlations that might realistically come up (eg between `company.country_code = "[jp]"` and `name.name LIKE "%Yu%"`) but the worst outliers have silly redundant constraints such as `title.title = 'Shrek 2'` and `keyword.keyword = 'computer-animation'`. 
